...
export class RegistroMaestrosComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.activatedRoute.snapshot.params['id'] != undefined){
      ...
      try {
        if (typeof this.maestro.materias_json === "string") {
          const reparado = this.maestro.materias_json.replace(/'/g, '"');
          this.maestro.materias_json = JSON.parse(reparado);
        }
      } catch (e) {
    ...
    // --- Asegurar que materias_json SIEMPRE sea arreglo ---
    if (!Array.isArray(this.maestro.materias_json)) {
      try {
        const reparado = (this.maestro.materias_json || "").toString().replace(/'/g, '"');
        this.maestro.materias_json = JSON.parse(reparado);
      } catch (e) {
        console.error("Error parseando materias_json:", e);
        this.maestro.materias_json = [];
  ...
  public checkboxChange(event: any) {
    ...
    if (event.checked) {
      ...
      this.maestro.materias_json = this.maestro.materias_json.filter(
        (materia: any) => materia !== event.source.value
      );
    }
